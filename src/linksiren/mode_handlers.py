...
from linksiren.impure_functions import read_targets, get_sorted_rankings, write_payload_local, \
                                       write_list_to_file, get_lnk_template
from linksiren.pure_functions import filter_targets, is_valid_payload_name, create_lnk_payload, \
...
def handle_rank(args, domain, username, password):
    ...
    sorted_rankings = get_sorted_rankings(targets, domain, username, password, threshold_date,
...
def handle_identify(args, domain, username, password):
    ...
    threshold_date = compute_threshold_date(datetime.now(), args.active_threshold)
    targets = read_targets(args.targets)
    sorted_rankings = get_sorted_rankings(targets=targets, domain=domain, username=username,
                                          password=password, active_threshold_date=threshold_date,
                                          max_depth=args.max_depth, go_fast=args.fast)
...
def handle_deploy(args, domain, username, password):
    ...
    for target in targets:
        ...
        for path in target.paths:
            write_successful = target.write_payload(path=path, payload_name=args.payload,
