...
def parse_args():
    ...
    generate_parser.add_argument('-n', '--payload',
    ...
    rank_parser = subparsers.add_parser('rank',
                                        help='Output identified subfolders and rankings to '
                                        'folder_rankings.txt')
    rank_required_group = rank_parser.add_argument_group('Required Arguments')
    rank_required_group.add_argument('credentials',
                                     help='[domain/]username[:password] for authentication')
    ...
    rank_parser.add_argument('-md', '--max-depth', type=int, default=3,
                             help='(Default: 3) The maximum depth of folders to search within '
                             'the target.')
    rank_parser.add_argument('-at', '--active-threshold', type=int, default=2,
                             help='(Default: 2) Number of days as an integer for active files.')
    rank_parser.add_argument('-f', '--fast', action='store_true', default=False,
    ...
    identify_required_group.add_argument('credentials',
                                         help='[domain/]username[:password] for authentication')
    ...
    identify_parser.add_argument('-md', '--max-depth', type=int, default=3,
                                 help='(Default: 3) The maximum depth of folders to search '
                                 'within the target')
    identify_parser.add_argument('-at', '--active-threshold', type=int, default=2,
                                 help='(Default: 2) Max number of days since within which a file is'
                                 ' considered active.')
    identify_parser.add_argument('-f', '--fast', action='store_true', default=False,
                                 help='(Default: False) Mark folders active as soon as one active '
                                 'file in them is identified and move on. Ranks are all set to 1.')
    identify_parser.add_argument('-mf', '--max-folders-per-target', type=int, default=10,
    ...
    deploy_required_group.add_argument('credentials',
                                       help='[domain/]username[:password] for authentication')
    ...
    deploy_required_group.add_argument('-a', '--attacker', required=True,
                                       help='Attacker IP or hostname to place in poisoned files.')
    deploy_parser.add_argument('-n', '--payload', default='@Test_Do_Not_Remove.searchConnector-ms',
    ...
    cleanup_parser = subparsers.add_parser('cleanup',
                                           help='Delete poisoned files from folder UNC paths '
                                           'specified in --targets')
    cleanup_required_group = cleanup_parser.add_argument_group('Required Arguments')
    cleanup_required_group.add_argument('credentials',
                                        help='[domain/]username[:password] for authentication')
    ...
    cleanup_parser.add_argument('-n', '--payload',
                                default='@Test_Do_Not_Remove.searchConnector-ms',
                                help='(Default: @Test_Do_Not_Remove.searchConnector-ms) Name '
                                'of payload file ending in .library-ms, .searchConnector-ms, '
                                '.lnk, or .url')
