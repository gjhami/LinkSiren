...
def test_handle_generate_valid_payload_name(args):
    ...
    with patch('linksiren.mode_handlers.is_valid_payload_name', return_value=True), \
         patch('linksiren.mode_handlers.get_lnk_template', return_value="lnk_template"), \
         patch('linksiren.mode_handlers.create_lnk_payload', return_value="payload_contents"), \
...
def test_handle_generate_payload_from_template(args):
    ...
    args.attacker = "attacker_ip"
    template_content = '[InternetShortcut]\n' + \
...
def test_handle_deploy_invalid_payload_name(args, domain, username, password):
    ...
    args.payload = "invalid_payload.txt"
    with patch('linksiren.mode_handlers.read_targets', return_value=["\\\\test_host\\test_share"]), \
...
def test_handle_deploy_write_payload(args, domain, username, password):
    ...
    with patch('linksiren.mode_handlers.is_valid_payload_name', return_value=True), \
        ...
        expected_payload_contents = template_content.format(attacker_ip=args.attacker)
        mock_target.write_payload.assert_called_once_with(path="test_path", payload_name=args.payload, payload=expected_payload_contents)
        mock_write_list_to_file.assert_called_once_with([r'\\test_host\test_path'], 'payloads_written.txt', 'a')
...
def test_handle_cleanup_multiple_targets(args, domain, username, password):
    ...
    with patch('linksiren.mode_handlers.read_targets', return_value=[mock_target1, mock_target2]):
        handle_cleanup(args, domain, username, password)
        mock_target1.connect.assert_called_once_with(user=username, password=password, domain=domain)
        mock_target1.delete_payload.assert_called_once_with("test_path1", args.payload)
        mock_target2.connect.assert_called_once_with(user=username, password=password, domain=domain)
